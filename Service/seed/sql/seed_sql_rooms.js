...
\n\n\n\n\n`;
  // create log file
  const log_file = path.join(__dirname, './seed-log/rooms.sql.log');
  const encoding = 'utf8';
...
const seed = async () => {
  ...
  for (let i = 1; i <= num_partitions; i++) {
    ...
    let command = `psql -h ${host} -p ${port} -U postgres -d airbnb -c "\\copy rooms (${fields}) FROM '${rooms_csv_absolute_path}' DELIMITER ',' CSV HEADER;" || exit 1`;
