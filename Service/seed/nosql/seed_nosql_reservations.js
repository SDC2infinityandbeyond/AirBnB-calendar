...
\n\n\n\n\n`;
  // create log file
  const log_file = path.join(__dirname, './seed-log/reservations.nosql.log');
  const encoding = 'utf8';
...
const seed = async () => {
  const host = process.env.MONGO_IMPORT_HOST || '127.0.0.1';
  const port = process.env.MONGO_IMPORT_PORT || 27017;
  ...
  for (let i = 1; i <= num_partitions; i++) {
    let drop = i === 1 ? '--drop ' : '';
    let reservations_csv_absolute_path = path.join(__dirname, `../csv/reservations${i}.csv`);
    let command =  `mongoimport --host ${host} --port ${port} --type csv -d airbnb -c reservations --headerline ${drop}--file "${reservations_csv_absolute_path}"`;;
