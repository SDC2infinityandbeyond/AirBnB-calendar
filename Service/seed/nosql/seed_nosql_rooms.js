...
const seed = async () => {
  ...
  for (let i = 1; i <= num_partitions; i++) {
    let drop = i === 1 ? '--drop ' : '';
    let rooms_csv_absolute_path = path.join(__dirname, `../csv/rooms${i}.csv`);
    let command =  `mongoimport --host ${host} --port ${port} --type csv -d airbnb -c rooms --headerline ${drop}--file "${rooms_csv_absolute_path}"`;
    sum_file_sizes += fs.statSync(rooms_csv_absolute_path).size;
